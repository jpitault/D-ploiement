...
"""
Script ou les ressources sont définit et nommé par leur adresse MAC,
Ca veut dire qu'il n'y a plus besoin de DELETE une ressource avant d'en ajouter une qui a la même adresse MAC
Le script supprime l'ancienne et tous les fichiers de conf qui ont été crée par l'ancienne ressource.
"""
...
storage_path='/root/ressourceApiMac'
...
class Hosts(object):
	...
	def on_post(self, req, resp):
		...
		if test != '':
			...
			# On veut que la ressource soit définit par la MAC donnée.
			...
			# Soit il y a déjà une ressource qui a ce nom
			if name in os.listdir(self._storage_path):
				# On doit non seulement la remplacer, mais aussi supprimer tous les fichiers qu'elle a crée.
				suph.menagexml(res_path)
				os.remove(res_path)
